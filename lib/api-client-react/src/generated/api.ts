 ...
 */
export const getResetStratumTokenUrl = () => {
  return `/api/me/stratum-token/reset`;
};

export const resetStratumToken = async (
  options?: RequestInit,
): Promise<MeProfile> => {
  return customFetch<MeProfile>(getResetStratumTokenUrl(), {
    ...options,
    method: "POST",
  });
};

export const getResetStratumTokenMutationOptions = <
  TError = ErrorType<UnauthorizedResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof resetStratumToken>>,
    TError,
    void,
    TContext
  >;
  ...
  TContext
> => {
  const mutationKey = ["resetStratumToken"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  const mutationFn: MutationFunction<
    ...
    void
  > = () => {
    return resetStratumToken(requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type ResetStratumTokenMutationResult = NonNullable<
  Awaited<ReturnType<typeof resetStratumToken>>
>;

export type ResetStratumTokenMutationError = ErrorType<UnauthorizedResponse>;

/**
 * @summary Regenerate the user's Stratum authentication token (invalidates all current rig connections)
 */
export const useResetStratumToken = <
  TError = ErrorType<UnauthorizedResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getResetStratumTokenMutationOptions(options));
};
